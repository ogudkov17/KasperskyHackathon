...
def verify_sign(data: str, sign: bytes):
    try:
        cert_file = open(certfile, "r")
        cert = cert_file.read()
        cert_file.close()
        ...
        if crypto.verify(certificate, signature, str(data), "sha256") is None:
            print("[info] Signature is correct")
            return True
    except Exception as e:
        print(f"[error] exception {e}")

    print("[error] Signature is incorrect")
    return False
...
def handle_event(id: str, details: dict):    
    ...
    try:
        if details['operation'] == 'verify_sign':
            data = details['data']
            sign = details['sign']
            print(f"[info] data {data}")
            verdict = verify_sign(data, sign)
            details['verdict'] = verdict
            details['deliver_to'] = details['source']
            details['operation'] = 'verified_sign'
            details['verified'] = None
            details['source'] = 'CDataVerifySign'
