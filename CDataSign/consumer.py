...
privatekeyfile = "./private/cert.key"

def sign_data(data: str):
    key_file = open(privatekeyfile, "r")
    key = key_file.read()
    key_file.close()
    
    pkey = crypto.load_privatekey(crypto.FILETYPE_PEM, key)
    sign = OpenSSL.crypto.sign(pkey, str(data), "sha256") 

    data_base64 = base64.b64encode(sign).decode('ascii')
    return data_base64
...
def handle_event(id: str, details: dict):    
    ...
    try:
        if details['operation'] == 'sign_data':
            data = details['data']
            print(f"[info] data {data}")
            data_b64 = sign_data(data)
            details['sign'] = data_b64
            details['deliver_to'] = details['source']
            details['operation'] = 'signed_data'
            details['verified'] = None
            details['source'] = 'CDataSign'
